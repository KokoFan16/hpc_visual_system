...
static std::chrono::time_point<std::chrono::system_clock> constructor_help(std::string name);
...
private:
	...
	void initial_str(std::string str) {
		if ( str.find(":") == -1 ) { tags = str; }
		else { selfdef = str; }
	}

public:
	Events(std::string n): name(n) { constr_help(n); }
	Events(std::string n, std::string str): name(n) { constr_help(n); initial_str(str); }
	Events(std::string n, std::string t, std::string pair): name(n), tags(t), selfdef(pair) { constr_help(n); }
	...
	Events(std::string n, int loop, int ite, std::string pair): name(n), is_loop(loop), loop_ite(ite), selfdef(pair) { constr_help(n); }
	Events(std::string n, std::string t, int loop, int ite, std::string pair): name(n), tags(t), is_loop(loop), loop_ite(ite), selfdef(pair) { constr_help(n); }
	...
	~Events() {
		...
		int found = namespath.rfind(delimiter);

		// set value (time and tag) of each function across all the time-steps
		if (curTs == 0 && output[namespath] == ""){
			output[namespath] += tags + "-" + std::to_string(is_loop);
			if (selfdef != "") {
				found = selfdef.find(":");
				if ( found == -1 || found == selfdef.size() - 1 ) 
					std::cout << "ERROR SELF-DEFINED KEY:VALUE (e.g., xx:xx;xx:xx)" << std::endl; 
				else {
					int pos = selfdef.rfind(";");
					std::cout << pos << std::endl;
					if ( pos == -1 || pos == selfdef.size() - 1) {
						std::string key = selfdef.substr(0, pos);
						std::string value = selfdef.substr(pos+1, selfdef.size());
						std::cout << key << ", " << value << std::endl;
					}
					// std::string token;
					// while ((pos = selfdef.find(";")) != std::string::npos) {
					//     token = selfdef.substr(0, pos);
					//     std::cout << token << std::endl;
					//     selfdef.erase(0, pos + 1);
					// }
					// std::string key = 
					// std::cout << pos << ", " << selfdef.size() <<  std::endl;
				}	
				
			// 	// std::cout << curRank << ", " << selfdef << std::endl;
			}
...
// 				if (i != nprocs - 1) { output[maxEvents[j]] += "|"; }
...
{
	// double sgather = MPI_Wtime();
	// int master = gather_info(); // gather info from all the processes
	// double egather = MPI_Wtime();
	// std::cout << curRank << ": gather time -- " << (egather - sgather) << std::endl;
